...
pub async fn show(
    ...
    let u: Option<User> = sqlx::query_as::<_, User>(crate::db::pg(
    ...
    let (follower_count,): (i64,) = sqlx::query_as(crate::db::pg(
        "SELECT COUNT(*) FROM follows WHERE followee_id = ?",
    ))
    .bind(u.id)
    .fetch_one(&state.pool)
    .await?;
    let (following_count,): (i64,) = sqlx::query_as(crate::db::pg(
    ...
    let viewer_follows = match &maybe_user.0 {
        Some(viewer) if viewer.id != u.id => {
            let (c,): (i64,) = sqlx::query_as(crate::db::pg(
