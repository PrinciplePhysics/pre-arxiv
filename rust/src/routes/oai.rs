...
fn error(code: &str, message: &str, base_url: &str) -> String {
    ...
    out.push_str(&format!("<error code=\"{}\">{}</error>\n", x(code), x(message)));
...
async fn identify(state: &AppState) -> String {
    let base_url = base(state);
    let earliest: Option<(NaiveDateTime,)> = sqlx::query_as(
    ...
    out.push_str(&format!("  <earliestDatestamp>{}</earliestDatestamp>\n", x(&earliest_str)));
    out.push_str("  <deletedRecord>persistent</deletedRecord>\n");
    out.push_str("  <granularity>YYYY-MM-DDThh:mm:ssZ</granularity>\n");
    out.push_str("  <description>\n");
    out.push_str("    <oai-identifier xmlns=\"http://www.openarchives.org/OAI/2.0/oai-identifier\"\n");
...
fn list_metadata_formats(base_url: &str) -> String {
    ...
    out.push_str("    <metadataNamespace>http://www.openarchives.org/OAI/2.0/oai_dc/</metadataNamespace>\n");
    out.push_str("  </metadataFormat>\n");
    out.push_str("</ListMetadataFormats>\n");
    out.push_str(&envelope_close());
    out
...
fn dc_metadata(r: &DbRow, base_url: &str) -> String {
    ...
    for a in r.authors.split(',').map(|s| s.trim()).filter(|s| !s.is_empty()) {
    ...
    out.push_str(&format!("    <dc:identifier>{}/m/{}</dc:identifier>\n", x(base_url), x(slug)));
    if let Some(d) = &r.doi {
        out.push_str(&format!("    <dc:identifier>doi:{}</dc:identifier>\n", x(d)));
    }
...
fn header_xml(r: &DbRow) -> String {
    ...
    let status_attr = if r.withdrawn != 0 { " status=\"deleted\"" } else { "" };
    let mut out = String::new();
...
async fn fetch_rows(
    ...
    let q = sqlx::query_as::<_, (Option<String>, String, String, String, String, Option<NaiveDateTime>, Option<String>, Option<String>, i64, Option<NaiveDateTime>)>(&sql);
    let rows = match set {
...
async fn list_identifiers(state: &AppState, q: &OaiQuery) -> String {
    let base_url = base(state);
    if let Some(err) = require_metadata_prefix(q, &base_url) { return err; }
...
fn require_metadata_prefix(q: &OaiQuery, base_url: &str) -> Option<String> {
    let pref = q.metadata_prefix.as_deref().or(q.metadata_prefix_alt.as_deref());
    match pref {
        Some("oai_dc") => None,
        Some(other) => Some(error("cannotDisseminateFormat", &format!("Unsupported metadataPrefix '{other}'; we support 'oai_dc'."), base_url)),
