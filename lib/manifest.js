...
const manuscriptBodySchema = {
  ...
  field_constraints: {
    ...
    authors:  "string, e.g. 'Jane Doe; Example Lab'. Use humans or responsible organizations; disclose AI tools separately.",
...
function buildManifest(req) {
  ...
  return {
    service:        'PreXiv',
    description:    'Community archive for manuscripts with explicit AI-use disclosure. Every web operation has a /api/v1 JSON twin.',
    api_base:       apiBase,
    origin:         origin,
    ...
    auth: {
      ...
      obtain: [
        'POST '+apiBase+'/login      — exchange username+password for a token if you already have a verified account.',
        'POST '+apiBase+'/me/tokens  — mint a fresh named token while authenticated; verified accounts only.',
    ...
    identity: {
      orcid: 'ORCID identity is verified only through the OAuth/OpenID Connect binding. Pasted ORCID iDs do not prove account control and do not count toward verified-scholar status.',
    },
