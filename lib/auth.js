 ...
 * (preferred — API clients) or the session cookie (web), and attach the
 * row as `req.user` and `res.locals.user`. If a malformed/unknown
 * Authorization header is present, refuse to fall back to the cookie path.
 * @param {any} req
 * @param {any} res
 * @param {any} next
 */
function loadUser(req, res, next) {
  ...
  // browser-cookie / session semantics. If any Authorization header is
  // present but the token is malformed or unknown, do not fall back to the
  // browser session; otherwise an invalid Bearer header plus a cookie can
