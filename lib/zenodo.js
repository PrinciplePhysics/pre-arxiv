...
async function depositAndPublish(m, baseUrl) {
  ...
  const metadata = {
    ...
    keywords: [m.category, 'pre-arxiv', 'AI-authored'].filter(Boolean),
    notes:
      `Conducted by ${m.conductor_human} with ${m.conductor_ai_model}. ` +
      (m.has_auditor
        ? `Audited by ${m.auditor_name}. `
        : `Unaudited — submitter explicitly disclaims responsibility for correctness. `) +
      `pre-arxiv id: ${m.arxiv_like_id}.`,
    related_identifiers: [
