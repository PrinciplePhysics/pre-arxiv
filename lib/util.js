...
/**
 * Render a "5 minutes ago" / "3 days ago" string from a SQLite datetime.
 * SQLite stores datetimes without a Z suffix; we treat them as UTC.
...
/**
 * Escape HTML special characters. Used in templates that interpolate
 * untrusted user content into attributes / text nodes.
 * @param {unknown} s
 * @returns {string}
 */
...
/**
 * Hacker-News-style rank: newer + higher score floats up.
 * @param {number} score
...
/**
 * Generate a fresh `prexiv:YYMM.NNNNN` id (5-digit serial random within month).
 * Year and month are taken from the supplied Date or "now".
 * @param {Date} [date]
 * @returns {string}
 */
function makeArxivLikeId(date = new Date()) {
  ...
  return `prexiv:${yy}${mm}.${serial}`;
}

/**
 * Render markdown to sanitised HTML. Only a fixed safe-tag whitelist is
 * allowed; <a> links are rewritten to rel="nofollow noopener" target="_blank".
 * @param {string} text
 * @returns {string}
 */
...
/**
 * Hours elapsed since a SQLite datetime. Treats naive timestamps as UTC.
 * @param {string} dateStr
 * @returns {number}
 */
...
/**
 * Read `page` and `per` from req.query and clamp them to sensible bounds.
 * @param {{query:{page?:any, per?:any}}} req
 * @param {number} [defaultPer]
 * @returns {{page:number, per:number, offset:number}}
 */
