...
/**
 * Split a manuscript's `authors` string into a list. Accepts ';', ' and ', or '&' as separators.
...
/**
 * Escape TeX-unsafe characters so the output BibTeX parses cleanly.
 * @param {unknown} s
 * @returns {string}
 */
...
/**
 * Render a BibTeX `@misc` entry for a manuscript.
 * @param {object} m manuscript row (must have title, authors, arxiv_like_id, created_at, …)
 * @param {string} baseUrl absolute origin used for the `url` field
 * @returns {string}
 */
function renderBibtex(m, baseUrl) {
  ...
  return [
    ...
    '  note          = {PreXiv: research preprints with AI-use disclosure; ' + conductorTag + privacySuffix(m) + (m.has_auditor ? '; audited by ' + m.auditor_name : '; unaudited') + '}',
    '}',
  ].filter(Boolean).join('\n');
}
...
/**
 * Render an RIS (Reference Manager) record for a manuscript.
 * @param {object} m manuscript row
 * @param {string} baseUrl absolute origin used for the `UR` line
 * @returns {string}
 */
function renderRis(m, baseUrl) {
  ...
  if (m.conductor_type === 'ai-agent') {
    const agentName = m.conductor_ai_model_public === 0 ? 'an undisclosed AI agent' : (m.conductor_ai_model || 'an AI agent');
    lines.push('N1  - Produced autonomously by ' + agentName + '; no human conductor');
  }
...
/**
 * Render a plain-text citation (one-line APA-ish).
 * @param {object} m manuscript row
 * @param {string} baseUrl absolute origin used for the trailing URL
 * @returns {string}
 */
